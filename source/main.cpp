    ...
    virtual tsl::elm::Element* createUI() {
        ...
        rootFrame->setHeader(new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
            ...
            if (edz::cheat::CheatManager::getProcessID() != 0) {
                renderer->drawString("TID:", false, 210 +14, 40 -6, 15, renderer->a(tsl::style::color::ColorText));
                renderer->drawString("BID:", false, 210 +14, 60 -6, 15, renderer->a(tsl::style::color::ColorText));
                renderer->drawString("PID:", false, 210 +14, 80 -6, 15, renderer->a(tsl::style::color::ColorText));
        ...
        if(edz::cheat::CheatManager::isCheatServiceAvailable()){
            auto cheatsItem = new tsl::elm::ListItem("作弊指令");
            ...
            auto noDmntSvc = new tsl::elm::ListItem("作弊功能不可用！");
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        rootFrame->setHeader(new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
            renderer->drawString("EdiZon", false, 20, 50+2, 32, renderer->a(tsl::defaultOverlayColor));
            renderer->drawString("作弊指令", false, 20, 50+23, 15, renderer->a(tsl::versionTextColor));
        ...
        if (edz::cheat::CheatManager::getCheats().size() == 0) {
            auto warning = new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h){
                ...
                renderer->drawString("未发现游戏或没有金手指！", false, 82, 360, 25, renderer->a(0xFFFF));
            });
            ...
            else list->addItem(new tsl::elm::CategoryHeader("可用作弊指令："));
            ...
            if(this->m_numCheats < 1){
                auto warning = new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h){
                    ...
                    renderer->drawString("未发现作弊指令！", false, 110, 340, 25, renderer->a(0xFFFF));
                });
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        auto infos = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, u16 x, u16 y, u16 w, u16 h){
            ...
            renderer->drawString("CPU  温度：", false, 63, 200, 18, renderer->a(tsl::style::color::ColorText));
            renderer->drawString("PCB  温度：", false, 63, 230, 18, renderer->a(tsl::style::color::ColorText));
    
            renderer->drawRect(x, 243, w, 1, renderer->a(tsl::style::color::ColorFrame));
            renderer->drawString("CPU  频率：", false, 63, 270, 18, renderer->a(tsl::style::color::ColorText));
            renderer->drawString("GPU  频率：", false, 63, 300, 18, renderer->a(tsl::style::color::ColorText));
            renderer->drawString("MEM  频率：", false, 63, 330, 18, renderer->a(tsl::style::color::ColorText));
    
            renderer->drawRect(x, 343, w, 1, renderer->a(tsl::style::color::ColorFrame));
            renderer->drawString("地址 IP：", false, 63, 370, 18, renderer->a(tsl::style::color::ColorText));
            ...
            if (this->m_ipAddressString ==  "0.0.0.0")
                renderer->drawString("离线", false, 258, 370, 18, renderer->a(tsl::style::color::ColorHighlight));
            else 
            ...
            if(hosversionAtLeast(15,0,0)){
                ...
                renderer->drawString("当前网络：", false, 63, 400, 18, renderer->a(tsl::style::color::ColorText));
                if(conStatus == NifmInternetConnectionStatus_Connected && conType == NifmInternetConnectionType_WiFi) {
                    std::string wifiStrengthStr = "(强)";
                    ...
                    renderer->drawString("以太网", false, 258, 400, 18, renderer->a(tsl::style::color::ColorHighlight));
                } else {
                    renderer->drawString("未连接", false, 258, 400, 18, renderer->a(tsl::style::color::ColorHighlight));
                }
            } else {
                s32 signalStrength = 0;
                wlaninfGetRSSI(&signalStrength);
    
                renderer->drawString("WiFi 信号：", false, 63, 400, 18, renderer->a(tsl::style::color::ColorText));
